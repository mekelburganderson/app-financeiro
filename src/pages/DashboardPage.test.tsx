...
function mount(){return render(<AuthContext.Provider value={auth}><BalanceVisibilityProvider><BalanceVisibilityToggle/><DashboardPage/></BalanceVisibilityProvider></AuthContext.Provider>)}
...
beforeEach(()=>{vi.clearAllMocks();localStorage.clear();vi.mocked(getDashboardData).mockResolvedValue(full)})
afterEach(()=>cleanup())
...
test('25: oculta saldos, limites e faturas sem ocultar os demais indicadores',async()=>{mount();await loaded();fireEvent.click(screen.getByRole('button',{name:'Ocultar saldos e limites'}));expect(screen.queryByText(/R\$\s*7\.500,00/)).toBeNull();expect(screen.queryByText(/R\$\s*5\.200,00/)).toBeNull();expect(screen.queryByText(/R\$\s*5\.000,00/)).toBeNull();expect(screen.queryByText(/R\$\s*1\.300,00/)).toBeNull();expect(screen.getAllByRole('img',{name:'Valor oculto'}).length).toBeGreaterThan(0);expect(screen.getAllByText(/R\$\s*4\.200,00/).length).toBeGreaterThan(0);fireEvent.click(screen.getByRole('button',{name:'Mostrar saldos e limites'}));expect(screen.getByText(/R\$\s*7\.500,00/)).toBeTruthy()})
